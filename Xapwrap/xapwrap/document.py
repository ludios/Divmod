...
class Document:
    ...
    @cvar _noObject: dummy object use to indicate that there is no
    data attribute
    ...
    def __init__(self, textFields = (), sortFields = (), keywords = (),
        ...
        xapian is concerned. We diffrerentiate them in the hope of
    ...
    def toXapianDocument(self, indexValueMap):
        ...
        for field in self.textFields:
            for token in analyzer.tokenize(field.text):
                # the tokenizer cannot garuntee that token length is
...
def makePairForWrite(prefix, token):
    ...
    if isinstance(token, str):
        token = token.decode(UNICODE_ENCODING, UNICODE_ERROR_POLICY)

    result = prefix.upper() + token
    # since return value is going into the db, it must be encoded as
    # UTF-8
